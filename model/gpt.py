...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.lin = nn.Linear(model_dim, vocab_size)
        self.softmax = nn.LayerNorm(model_dim)

    def forward(self, context: TensorType[int]) -> TensorType[float]:
        torch.manual_seed(0)
        # 1. Add token embeddings + position embeddings (use torch.arange for positions)
        embedding = self.token_embed(context)
        positions = torch.arange(context.shape[1], device = context.device)
        embedding = embedding + self.pos_embed(positions)
        
        # 2. Pass through transformer blocks
        x = self.transformer_block(embedding)
